...
RSpec.feature 'User feature', type: :feature do
  ...
  describe '#index' do
    ...
    let!(:booking) { create(:booking, activity:, user:) }
    let!(:log) { create(:log, activity:, user:, booking:, type: 'submit') }
  ...
  describe '#show' do
    ...
    let!(:activity) { create(:activity, user:) }
    let!(:booking) { create(:booking, activity:, user:) }
    let!(:booking) { create(:booking, activity:, user: user2) }
    let!(:logs) do
      [
        create(:log, user:, type: 'submitted', created_at: 1.hour.ago),
        create(:log, user:, type: 'login', created_at: 2.hour.ago),
        create(:log, user:, type: 'browse_activity_show', created_at: 2.hour.ago),
        create(:log, user:, type: 'email_field_changed', created_at: 2.hour.ago),
        create(:log, user:, type: 'submitted', created_at: 2.hour.ago)
      ]
    end
    ...
    scenario 'Logged in (admin)' do
      ...
      expect(page).to have_content("使用者【#{user.name}】活動詳情")
