...
RSpec.feature 'Activity feature', type: :feature do
  describe '#index' do
    let!(:activity1) do
      create(:activity, title: '默默會一', description: '五倍學員聚聚一', start_time: 1.day.from_now, location: '地點一')
    end
    let!(:activity2) do
      create(:activity, title: '默默會二', description: '五倍學員聚聚二', start_time: 2.days.from_now, location: '地點二')
    end
  ...
  describe '#show' do
    ...
    scenario 'nonmember or User is not logged in tries to book an activity' do
      FactoryBot.create(:booking, activity:, user:)
